...
- Verification performed: confirmed both new Bash scripts pass `bash -n` syntax validation inside a clean `bash:5.2` container; reviewed the updated checklist and runbook for consistency with the live production domains and current deployment flow; confirmed GitHub Actions runs `24812385648`, `24812561565`, and `24812822312` completed successfully while iterating on the scripts; confirmed `bash deploy/demo-preflight.sh` succeeds on the EC2 host; confirmed `bash deploy/demo-simulate-recovery.sh catalog_service https://tungtungtungtungsahur.site` succeeds on the EC2 host and restores the service
- Remaining blockers: optional Terraform drift and optional Ansible remain open; evidence screenshots/logs still need to be captured
- Next recommended step: capture the evidence bundle and rehearse the full final demo using the new scripts

## [2026-04-23] Tier 5 Kubernetes cutover completed and recovered after lab reset

- Summary: Upgraded the live production system from the earlier Docker Compose baseline to Tier 5 on k3s Kubernetes, migrated the live MySQL data into a Kubernetes StatefulSet, cut public Nginx traffic over to Kubernetes NodePorts, updated the CI/CD workflow toward the new deploy path, and then verified the whole stack recovered cleanly after a Learner Lab reset and instance restart.
- Rubric areas affected: architecture, deployment orchestration, CI/CD, monitoring, demo readiness, operational continuity
- Files touched: `services/booking/main.py`, `services/booking/database.py`, `services/catalog/database.py`, `services/identity/main.py`, `services/management/database.py`, `services/otp/database.py`, `services/payment/main.py`, `services/payment/database.py`, `services/scheduler/database.py`, `deploy/k3s/install-k3s.sh`, `deploy/k3s/deploy-k3s.sh`, `deploy/k3s/db.yaml.tmpl`, `deploy/k3s/apps.yaml.tmpl`, `deploy/k3s/frontend-default.conf`, `deploy/k3s/setup-nginx-k3s.sh`, `deploy/k3s/restore-nginx-compose.sh`, `monitoring/prometheus/prometheus-k3s.yml`, `.github/workflows/ci-cd.yml`, `deploy/demo-preflight.sh`, `deploy/demo-simulate-recovery.sh`, `docs/DEMO_RUNBOOK.md`, `docs/PRODUCTION_CHECKLIST.md`, `AGENT.md`, `HANDOFF.md`, `MILESTONE_LOG.md`
- Verification performed: confirmed `python -m compileall services` succeeds after the Kubernetes compatibility changes; confirmed the k3s scripts pass `bash -n`; confirmed the k3s manifest templates render successfully with `envsubst`; confirmed k3s installed successfully on the EC2 instance and the node is `Ready`; confirmed the `moviebooking` namespace has healthy pods, services, and PVCs for the app, MySQL, Prometheus, Grafana, node-exporter, and cAdvisor; confirmed Prometheus inside Kubernetes reports `prometheus`, `node-exporter`, and `cadvisor` as active `up` targets; confirmed host Nginx proxies the public domains to Kubernetes NodePorts; confirmed public HTTPS app, Adminer, and Grafana remain healthy after shutting down the legacy Compose stack; confirmed `bash deploy/demo-preflight.sh` and `bash deploy/demo-simulate-recovery.sh catalog-service https://tungtungtungtungsahur.site` succeed against the live Kubernetes stack; confirmed SSH, public HTTPS, Grafana health, and the Kubernetes preflight all recovered successfully after a Learner Lab reset and instance restart; confirmed the remaining GitHub Actions Tier 5 deploy failure was only a missing executable bit on `setup-nginx-k3s.sh` after `scp` upload, and verified the fix by rerunning `CUTOVER_NGINX=true bash deploy/k3s/deploy-k3s.sh deploy/.env.production` successfully on the EC2 host.
- Remaining blockers: GitHub Actions still needs one fresh post-fix run to prove the full hosted Tier 5 `deploy-production` path is green after the `bash` cutover fix; optional Terraform drift and optional Ansible remain open; evidence screenshots/logs still need to be captured.
- Next recommended step: push the cutover-fix commit, wait for the next GitHub Actions run to complete, and confirm `deploy-production` passes end-to-end on the k3s path.

## [2026-04-23] Hosted Tier 5 GitHub Actions deployment verified end-to-end

- Summary: Hardened the hosted Tier 5 deploy workflow so GitHub Actions normalizes the SSH key secret, validates it on the runner, waits for SSH readiness, and then deploys to the k3s production stack reliably; pushed the fix and confirmed the next hosted run completed successfully end-to-end.
- Rubric areas affected: CI/CD, deployment orchestration, architecture validation, demo readiness, operational continuity
- Files touched: `.github/workflows/ci-cd.yml`, `AGENT.md`, `HANDOFF.md`, `MILESTONE_LOG.md`
- Verification performed: confirmed the production EC2 instance recovered fully after the Learner Lab restart; confirmed SSH access, public app HTTPS, and Grafana health all returned before the push; confirmed GitHub Actions run `24817015545` completed successfully including `deploy-production`; confirmed the live Kubernetes application deployments now track GHCR image tag `cc040ef86d50eb0de5caefa7a37a80f2db25e78e`; confirmed `bash deploy/demo-preflight.sh` still succeeds on the EC2 host after the hosted deploy.
- Remaining blockers: optional Terraform in-place drift and optional Ansible remain open; evidence screenshots/logs still need to be captured.
- Next recommended step: capture the final evidence set and rehearse the final Tier 5 demo flow while the fully hosted deploy path is green.
