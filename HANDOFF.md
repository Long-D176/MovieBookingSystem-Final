...
- optional Ansible
- first successful GHCR-based production deployment run from GitHub Actions
...
- The source-built fallback deployment is running now, but the graded CI/CD path still depends on GitHub Actions secrets before GHCR publishing/deployment can be tested.
...
2. rerun the CI/CD workflow and verify the GHCR-based image path now that the Python 3.11 image hardening fixes are committed
