...
def get_my_bookings(user_id: int = Depends(get_current_user_id), db: Session = Depends(get_db)):
    ...
    for b in bookings:
        ...
        qr_codes = [t.qr_code for t in tickets]

        concessions_data = db.query(models.BookingConcession).filter(models.BookingConcession.booking_id == b.booking_id).all()
        concessions_list = [{"item_id": c.item_id, "quantity": c.quantity, "price": c.price} for c in concessions_data]
...
def create_booking(req: BookingRequest, 
    ...
    try:
        ...
        screen_id = showtime_res.json()['screen_id']
        
        seats_res = httpx.get(f"{CATALOG_SERVICE_URL}/screens/{screen_id}/seats")
        screen_seats_map = {s['seat_id']: s['seat_type_id'] for s in seats_res.json()} if seats_res.status_code == 200 else {}
    ...
    for seat_id in req.seat_ids:
        type_id = screen_seats_map.get(seat_id, 1)
        surcharge = seat_type_surcharges.get(type_id, 0.0)
        real_price = BASE_PRICE + surcharge
